...
def get_raw_ts(fname_labels="labels_2020_sep_30.dms",fname_data="sensedog_2020_feb03_ts_lgb_transition_final.csv"):
    ...
    data_filt['user_acceleration_x/z'] = data_filt['user_acceleration_x'] / data_filt['user_acceleration_z']
    data_filt['user_acceleration_abs'] = np.linalg.norm(data_filt[['user_acceleration_x', 'user_acceleration_y', 'user_acceleration_z']].values, axis=1)
    data_filt['rotation_rate_abs'] = np.linalg.norm(data_filt[['rotation_rate_x', 'rotation_rate_y', 'rotation_rate_z']].values, axis=1)
    del data
...
#         print(len(df_ua.columns))
#         print(df_ua)
    df_ua.columns = ['timestamp', 'user_acceleration_x', 'user_acceleration_y', 'user_acceleration_z']
#     print(df_ua.info())

    df_at = getvalues(json_data, 'attitude')
#     df_at = pd.DataFrame(attitude)
    df_at.head(5)
    df_at.columns = ['timestamp', 'attitude_1', 'attitude_2', 'attitude_3', 'attitude_4'] 
#     print(df_at.info())
...
#     df_rr = pd.DataFrame(rotation_rate)
    df_rr.head(5)
    df_rr.columns = ['timestamp', 'rotation_rate_x', 'rotation_rate_y', 'rotation_rate_z']


    label_time = df['Time'].apply(lambda x: syncpoint + td(seconds = x))
    for index, row in df.iterrows():
        if (row["Forelimb(s)"] in ['fore_rasp', 'dig'] or 
            row['Hindlimb(s)'] in [ 'hind_rasp', 'hind_bend'] or 
            row['Body'] in ['roll']):
        ...
        if row["Posture/Gait"] in ['lay_side', 'lay_rightside', 'lay_leftside', 'lay_curled', 'lay_back']:
...
def convert_to_ts_conv(filename,path,conv=""):
    ...
    for index, row in df.iterrows():
        if (row["Forelimb(s)"] in ['fore_rasp', 'dig'] or 
            row['Hindlimb(s)'] in [ 'scratch', 'hind_rasp', 'hind_bend'] or 
            row['Body'] in ['body_shake', 'roll'] or
            row['Nose/Mouth'] in ['chew']):
            df.loc[index,'Posture/Gait'] = np.NaN
        if row['Nose/Mouth'] == "eat":
            df.loc[index,'Posture/Gait'] = "eat"
        if row['Nose/Mouth'] == 'drink':
...
def stat_features(data_filt,cat_inv,dog_inv,cat_pol='last',window=32,shift=16,verbose=False):
    ...
    for i in range(len(ndogs)):
        ...
        if verbose==True:
            print("%dth dog samples: %d dimension: %d (timeseries: %d) name: %s"%(i,ts_len,dim,nts,ndogs[i]))
    ...
    for i in range(len(full)):
        if i>0:
            if int(full[i,2])!=last:
                 ...
                 num+=1
        last=int(full[i,2])
        print("%d|%d|%s|%d|%s|%d" % (i,num,dog_occ.index[int(full[i,2])],int(full[i,2]),labels[labels.values[:,0]==int(full[i,3])].values[0,1],int(full[i,3])), end="",file=f)
...
def full_context(new_meta,cx=2,cy=2,old=False):
    ...
    for k in range(n_dogs):
        cont_base = new_meta[new_meta['name']==dog_names[k]].values[:,3:].astype(np.float32)
        cont_feat[act_pos:act_pos+len(cont_base)] = prob_context(cont_base,cx,cy)
        cont_y[act_pos:act_pos+len(cont_base)] = new_meta[new_meta['name']==dog_names[k]].values[:,2].astype(np.float32)
    ...
    if old==False:
        ...
        cont_feat_old= cont_feat[(cont_y[:]==2) | (cont_y[:]==3) | (cont_y[:]==4) | (cont_y[:]==9) | (cont_y[:]==10) | (cont_y[:]==11)]
        cont_y_old= cont_y[(cont_y[:]==2) | (cont_y[:]==3) | (cont_y[:]==4) | (cont_y[:]==9) | (cont_y[:]==10) | (cont_y[:]==11)]
        cont_dog_id_old= cont_dog_id[(cont_y[:]==2) | (cont_y[:]==3) | (cont_y[:]==4) | (cont_y[:]==9) | (cont_y[:]==10) | (cont_y[:]==11)]
...
def keras_train(model,x_train,y_train,batch_size=32,nb_epoch=1,data_augmentation=False):
...
def keras_eto_rnn_mlp(input_dim, input_len, n_cla=28, rec_layer=16, hidd=32, layer_type='gru'):
    model = Sequential()
